...
def criar_tela_login():
    ...
    layout = [
        ...
        [sg.Text("Salt:           "),sg.Input(key="salt", size=(20,1),default_text='otimizemeadmin')],
...
def monitorar_eventos(window):
    while True:
        ...
        if event == "ok":
            ...
            gerou_erro = verificar_configuracoes_hash(valor_alphabet, valor_min_lenght, hashid)
...
def verificar_configuracoes_hash(valor_alphabet, valor_min_lenght, hashid, gerou_erro = False):
    global valor_alphabet_corrigido
    mensagem_erro = "Erro(s): \n"
    if valor_alphabet:
        valor_alphabet = set(valor_alphabet)
        valor_alphabet_corrigido = ''
        for caracteres in valor_alphabet:
            valor_alphabet_corrigido += caracteres
    else:
        gerou_erro = True
        mensagem_erro += "-O Campo Alphabet deve conter 16 caracteres \n"
    try:
